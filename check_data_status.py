    ...
    try:
        ...
        if 'COUNTYNAME' in gdf.columns:
            ...
            for i, county in enumerate(counties):
                ...
                print(f"     {i+1:2d}. {county} ({count} 個鄉鎮)")
...
def check_coordinate_systems(gdf, raster_info):
    """檢查座標系統一致性"""
    print("\n=== 檢查座標系統一致性 ===")
    
    if gdf is None:
        print("❌ 無法檢查座標系統 (鄉鎮邊界載入失敗)")
        return False
    
    # 檢查鄉鎮邊界座標系統
    town_crs = gdf.crs
    print(f"鄉鎮邊界座標系統: {town_crs}")
    
    # 檢查柵格座標系統
    raster_crs_list = []
    for filename, info in raster_info.items():
        if info is not None:
            raster_crs = info['crs']
            raster_crs_list.append(raster_crs)
            print(f"{filename}: {raster_crs}")
    
    if not raster_crs_list:
        print("❌ 沒有有效的柵格資料")
        return False
    
    # 檢查一致性
    all_same = all(str(crs) == str(raster_crs_list[0]) for crs in raster_crs_list)
    
    if all_same:
        target_crs = raster_crs_list[0]
        print(f"✅ 所有柵格座標系統一致: {target_crs}")
        
        if str(town_crs) != str(target_crs):
            print(f"⚠️ 鄉鎮邊界座標系統不同，需要轉換")
            print(f"   鄉鎮: {town_crs}")
            print(f"   柵格: {target_crs}")
            return False
        else:
            print(f"✅ 所有座標系統一致")
            return True
...
def main():
    ...
    # 檢查座標系統
    coords_ok = check_coordinate_systems(gdf, raster_info)
    ...
    return gdf, target_counties, raster_info, coords_ok

if __name__ == "__main__":
    gdf, target_counties, raster_info, coords_ok = main()
